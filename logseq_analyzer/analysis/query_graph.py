...
class Query:
    ...
    def name_to_hashes(self, name: str):
        ...
        return self.graph.file_index.name_to_hashes.get(name, [])

    def hash_to_file(self, hash_: int):
        """Get a Logseq file by its hash."""
        return self.graph.file_index.hash_to_file.get(hash_, None)

    def name_to_files(self, name: str):
        """Get Logseq files by name."""
        return self.graph.file_index.name_to_files.get(name, [])

    def hash_to_name(self, hash_: int):
        """Get a Logseq file name by its hash."""
        if not (path := self.graph.file_index.hash_to_file.get(hash_)):
            return None
    ...
    def list_files_with_keys_and_values(self, **criteria) -> List[LogseqFile]:
        ...
        result = []
        for _, file in self.graph.file_index.hash_to_file.items():
