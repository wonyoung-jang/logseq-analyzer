...
def test_get_datetime_token_map(datetime_tokens):
    ...
    assert all(isinstance(v, str) for v in datetime_tokens._token_map.values()), "Values should be strings."
    assert all(len(k) > 0 for k in datetime_tokens._token_map.keys()), "Keys should not be empty strings."
    assert all(len(v) > 0 for v in datetime_tokens._token_map.values()), "Values should not be empty strings."
    assert all(k != v for k, v in datetime_tokens._token_map.items()), "Keys and values should not be the same."
    assert all(k != "" for k in datetime_tokens._token_map.keys()), "Keys should not be empty strings."
...
def test_set_datetime_token_pattern(datetime_tokens):
    ...
    assert len(datetime_tokens._token_pattern.pattern) > 0, "Datetime token pattern should not be empty."
    assert all(isinstance(k, str) for k in datetime_tokens._token_map.keys()), "Keys should be strings."
    assert all(isinstance(v, str) for v in datetime_tokens._token_map.values()), "Values should be strings."
    assert all(len(k) > 0 for k in datetime_tokens._token_map.keys()), "Keys should not be empty strings."
    assert all(len(v) > 0 for v in datetime_tokens._token_map.values()), "Values should not be empty strings."
